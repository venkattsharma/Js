...
Parsing :- Human readable code converted into Abstract syntax tree ( tokens )
Compilation :- Just in time compilation ( compiler + interpreter ) optimised code.
Execution :- Starts executing the code with the help of call stack and memory heap.
        When javaScript engine starts executing the code, It creates the global execution context in callstack
Each context in call stack has two phases:-
1) Memory Creation
2) Code Execution
...
case 2:- let and const variables ( Temporal Dead Zone :- Technically they are also hoisted )
 What is TDZ ? :- time frame between let and const variables are hoisted to they are initialized
 let and const are allocated in different memory space ( script scope ) than global scope
 */
